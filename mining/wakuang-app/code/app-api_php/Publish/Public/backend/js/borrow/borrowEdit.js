...
function changeSelect(value) {
    ...
    if (current.typeCode === "600") {
        $("#hidUserDiv").hide();
        $("#userID").val("0");
        $("#borrowUserID").val("0");
        $("#userID").rules("remove");
        $("#borrowUserID").rules("remove");
    } else {
        $("#hidUserDiv").show();
        $("#userID").rules("add", {required: true,
            messages: {
                required: "请选择平台融资人"
            }
        });
        $("#borrowUserID").rules("add", {required: true,
            messages: {
                required: "请选实际借款人"
            }
        });
...
function saveBorrow() {
    var data = $Util.getData($("#form-Borrow"));
    data.realName = $("#userID").find("option:selected").text();
...
function BorrowValid() {
    ...
    $.validator.addMethod("feesMoney", function (value, element) {
        if (value * 1 < 0) {
            return false;
        }
    ...
    $("#form-Borrow").validate({
        ...
        messages: {
            ...
            borrowAprSub: {Apr: "利率格式不正确"},
            totalReceipts: {feesMoney: "总收益格式不正确"},
