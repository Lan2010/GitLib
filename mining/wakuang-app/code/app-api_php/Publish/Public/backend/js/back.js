        ...
        success: function(data) {
            if (data.length > 0) {
                ...
                $.each(data, function(index, item) {
                    mhtml += "<a href='" + item.menu_url + "' onclick='submenuclick(this, event)' class='sub-menu-sidebar'>" + item.menu_name + "</a>";
                });
                $('#ulhtml').html(mhtml + "</li>");
                $(".sidebar-sub-nav").hideLoading();
                $(".sub-content").showLoading();
                $(".sub-content").load(data[0]["menu_url"], null, function(response, status) {
                    if (status == "success") {
                        $(".sub-content").fadeIn('slow');
                        $(".sub-content").hideLoading();
                    } else {
