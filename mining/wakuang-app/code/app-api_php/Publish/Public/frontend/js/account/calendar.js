...
/*************    方法     **************/
	...
	function kalendarCode(codeYear,codeMonth,codeDay,codeFirst){
		...
		//年 选择
		for(var i=2014;i<=codeYear+yearfloor;i++){
			if(i == codeYear){
		...
		for(var m=0;m<6;m++){//日期共 4-6 行
			if(m >= Math.ceil((codeFirst+monthDays[codeMonth])/7)){//第五、六行是否隐藏				
				kalendar_html += "<ul class='dayList dayListHide"+m+" '  style='display:none'  >\n";
			}else{
			...
			for(var n=0;n<7;n++){//列
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					kalendar_html += "<li class='calendar1'></li>";
				}else{
					if((7*m+n+1-codeFirst == codeDay)&&(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6))){//当天是周末
					//	kalendar_html += "<li id='day_"+(7*m+n+1-codeFirst)+"' class='todayWeekend'>"+(7*m+n+1-codeFirst)+"</li>";
                                                kalendar_html += "<li id='day_"+(7*m+n+1-codeFirst)+"' class='weekend'>"+(7*m+n+1-codeFirst)+"</li>";
					}else if(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6)){//仅是周末
						kalendar_html += "<li id='day_"+(7*m+n+1-codeFirst)+"' class='weekend'>"+(7*m+n+1-codeFirst)+"</li>";
					}else if(7*m+n+1-codeFirst == codeDay){//仅是当天
						kalendar_html += "<li id='day_"+(7*m+n+1-codeFirst)+"'class=''>"+(7*m+n+1-codeFirst)+"</li>";
					}else{//其他日期
						kalendar_html += "<li id='day_"+(7*m+n+1-codeFirst)+"' class=''>"+(7*m+n+1-codeFirst)+"</li>";
					}
				}
	...
	//修改日历列表 的方法
	//参数依次为 操作对象(每一天) 月份 修改后的第一天是周几 修改后的总天数 当天的具体日期
	function dateChange(dateObj,dateMonth,dateFirstDay,dateTotalDays,dateCurrentDay){
		...
		*如当前3.31，选中2月，可2月最多29天，则将当前日期改为选中月的最后一天，即2.29
		*/
		...
		for(var i=0;i<7*newLine;i++){
			if(i < dateFirstDay || i> (dateTotalDays+dateFirstDay-1)){//日历中 当月不存在的日期
				...
				dateObj.eq(i).attr( "id",("day_"+(i+1-dateFirstDay)));
			}
		}
	}
...
/*************   将日历代码放入相应位置，初始时显示此处内容      **************/
	...
	// alert('年:'+someDay['year']+'\n月:'+someDay['month']+'\n日:'+someDay['date']+'\n周:'+someDay['week']+'\n本月第一天周:'+someDay['firstDay']);return false;
