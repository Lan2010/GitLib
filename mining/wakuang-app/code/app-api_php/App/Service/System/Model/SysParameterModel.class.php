...
class SysParameterModel extends SlaveModel {
    ...
    public function getParameter($where, $whereArr, $limit) {
        ...
        $result["rows"] = $this->table("d_sys_parameter")->where($where, $whereArr)->order("add_datetime DESC")->limit($limit)->select();
        return $result;
    }
    ...
    public function getParMore($where) {
        $field = "par_key,par_value";
        $result = $this->field($field)->where($where)->order("add_datetime DESC")->select();
        return $result;
    }
    ...
    public function saveParameter($where, $data) {
        try {
            ...
            if (!is_null($where) && count($where) > 0) {
                ...
                return $result * 1 ? $where["par_id"] : false;
            } else {
