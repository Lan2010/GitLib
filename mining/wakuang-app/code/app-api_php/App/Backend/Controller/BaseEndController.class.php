...
class BaseEndController extends BaseController {
    ...
    /**
     * 保存文件原先系统上传 现在不用了 作为一个备份
     * @param type $rootPath 第一级文件名
     * @param type $subPath 最后一级文件名
     * @return \Think\Upload
     */
    public function uploadImg_dat($rootPath, $subPath = "") {
     ...
     * --
     */
    public function uploadImg($rootPath, $subPath = "", $del = "") {
        $imgInfo = explode(UPLOAD, $del);
        $del = is_array($imgInfo) && !empty($imgInfo) ? $imgInfo[1] : ""; // 1用户上传新头像将删除之前的头像
        $file = (!empty($_FILES["file"]))? $_FILES["file"]:$_FILES['Filedata'];
