...
class UploadClient {
    ...
    public function postImg($data,$url="") {
        if(file_exists('/'.$data['path'])) {
            return "4001:文件已经存在:/".$data['name'];
        } else {
            $dir  =  $_SERVER['DOCUMENT_ROOT'].'/'.dirname($data['path']);
            $filepath = $_SERVER['DOCUMENT_ROOT'].'/'.$data['path'];
            if(!file_exists($dir)) {
                mkdir($dir,0777,true);
            }
            $res = move_uploaded_file($data['file'],$filepath);
            if($res) {
                return json_encode(array("ret_code"=> "0000", "ret_msg"=> "上传成功"));
            } else {
                return "4002:报错啦:/".$data['name'];
            }
        }
    }
...
//        curl_setopt($curl, CURLOPT_SAFE_UPLOAD, false); //必须放在数据包前面
