...
abstract class MobileApiBase extends BaseApi {
    ...
    protected function userCheck($isExit = true) {
        ...
        if (empty($userInfo) || $userInfo['expires_time'] <= $_SERVER['REQUEST_TIME']) {
            if ($isExit) {
    ...
    protected function initUserInfo() {
        ...
        if (!empty($userInfo)) {
            ...
            $this->userDeviceID = $userInfo['device_id'];
            $this->userToken = $userInfo['token'];
            $this->loginTime = $userInfo['login_time'];
            $this->expiresTime = $userInfo['expires_time'];
            return;
        }
        $this->userID = 0;
        $this->userPhone = '';
    ...
    protected function setUserInfo($entity, callable $callback = null) {
        ...
        $loginKey = $this->getLoginCaheKey($entity['token'], $userID);   //存起来 token
