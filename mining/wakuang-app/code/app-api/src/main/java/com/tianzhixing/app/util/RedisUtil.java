...
public class RedisUtil {
	 ...
	 * Redis服务器IP
	 */
	private static String IP = CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.ip");
	/**
	 * 密码
	 */
	private static String PASSWORD = CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.password");
	/**
	 * Redis的端口号
	 */
	private static Integer PORT = Integer
			.valueOf(CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.port"));
	/**
	 * 可用连接实例的最大数目，默认值为1024；
	 * 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
	 */
	private static Integer MAX_TOTAL = Integer
			.valueOf(CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.maxTotal"));
	/**
	 * 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
	 */
	private static Integer MAX_IDLE = Integer
			.valueOf(CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.maxIdle"));
	/**
	 * 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
	 */
	private static Integer MAX_WAIT_MILLIS = Integer
			.valueOf(CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.maxWaitMillis"));
	/**
	 * 超时时间
	 */
	private static Integer TIMEOUT = Integer
			.valueOf(CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.timeOut"));
	/**
	 * 在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
	 */
	private static Boolean TEST_ON_BORROW = Boolean
			.valueOf(CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.testOnBorrow"));

	/**
	 * 键的过期时间,现在定为永久有效
	 */
	public static Integer EXPIRE = Integer
			.valueOf(CommonUtils.getPropertiesValue(Constant.APPLICATION_CONFIG_PATH, "redis.expire"));
	...
	public static Set<String> keys(String key) {
		...
		try {
			if (jedis != null)
				v = jedis.keys(key);// 取第一个到最后一个元素，-2：倒数第二个元素
		} catch (Exception e) {
			log.error(e.getMessage());
		} finally {
			closeJedis(jedis);
	 ...
	 * @param keys
	 */
	public static void dels(Set<String> keys) {
		Jedis jedis = getJedis();
		try {
			if (jedis != null) {
				for (String key : keys) {
